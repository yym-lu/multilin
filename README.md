...
### 核心实验设计：模拟低资源环境
虽然中文本身拥有海量数据，但本项目**创新性地构建了模拟低资源环境**——通过将中文训练数据限制在极小规模（4k/8k样本），来模拟真实业务中新语言缺乏标注数据的困境。同时，利用全量数据（20k）训练的模型作为性能天花板（Upper Bound），从而精确量化跨语言策略的收益。

### 关键结论
通过引入英语数据（OLID）辅助训练，我们证明了“8k中文 + 英文辅助”的模型（F1: 0.908）可以达到与“20k纯中文”模型（F1: 0.909）几乎一致的效果。
这意味着：**在保证模型性能的前提下，我们成功节省了 60% 的中文人工标注成本。**

这一结论为AI产品在多语言场景下的低成本冷启动提供了强有力的数据策略支持。

## 核心特性

- **基座模型**: XLM-Roberta-base (支持多语言)
- **微调技术**: LoRA (轻量级微调)
- **数据策略**: 跨语言数据增强 (中文 + 英文)
